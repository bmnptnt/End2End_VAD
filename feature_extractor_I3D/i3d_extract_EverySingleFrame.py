...
def run(args_item):
    ...
    # setup the model  
        ...
        stride=8
        clipped_length = math.ceil(frame_cnt/stride)
        ...
        for i in range(clipped_length):
            ...
            chunk=[]
            for j in range(i*stride - chunk_size//2, i*stride + chunk_size//2):
                if j<0:chunk.append(0)
                elif j>=frame_cnt:chunk.append(frame_cnt-1)
                else : chunk.append(j)
            frame_indices.append(chunk)
