...
def run_I3D_feature_extraction(input_folder,output_folder,level):

    parser = argparse.ArgumentParser()
    parser.add_argument('--mode', default="rgb",type=str)
    parser.add_argument('--load_model',default="./feature_extractor_I3D/model_rgb.pth", type=str)
    parser.add_argument('--input_dir', default="../dataset/frame/UCF-Testing",type=str)
    parser.add_argument('--output_dir',default="../dataset/feature/UCF-Testing", type=str)
    ...
    if level==1:
        for video in os.listdir(args.input_dir):
            ...
            if save_file in os.listdir(os.path.join(args.output_dir)):
                ...
                vid_list.append(os.path.join(args.input_dir, video))
    elif level==2:
        ...
        print(f"Structure of Frame folder is wrong.")
        return
    ...
    # st=time.time()
    # run(args.load_model, vid_list[0], args.output_dir,args.batch_size,nums)
