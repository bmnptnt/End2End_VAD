...
def infer_func(model, dataloader, gt, logger, cfg):
    ...
    with torch.no_grad():
        ...
        for i, (v_input, name, feat_path) in enumerate(dataloader):
            ...
            fpre_ = torch.zeros(logits.shape[0] * STRIDE).cuda()
            for i in range(fpre_.shape[0]):
                if i >= (logits.shape[0] - 1) * STRIDE:
                    fpre_[i] = logits[i // STRIDE]
                else:
                    fpre_[i] = (logits[i // STRIDE] + logits[(i // STRIDE) + 1]) / 2
            logits=fpre_
    ...
    for i in tqdm(range(len(frame_list))):
        ...
        plt.savefig(f'./graph/sw8/{video}.pdf')
        plt.close(score_fig)
