...
def find_threshold(th, pred, gt):
    f1_scores = []
    for i in tqdm(range(0, len(th), 100)):
    ...
    best_thd = th[max_f1_index * 100]
    best_f1 = f1_scores[max_f1_index]
    print(f"Best F1-Score : {max(f1_scores)} , Best Threshold : {best_thd} , Best THD Index : {max_f1_index * 100}")


def generate_graph(frame_list, video_list, pred16, gt):
    ...
    for i in tqdm(range(len(frame_list))):
        ...
        plt.plot(frame_num, pred16[start_frame:end_frame], label=video, color='b')
        ...
        plt.savefig(f'./graph/seg16/{video}.pdf')
        plt.close(score_fig)
...
def infer_func(model, dataloader, gt, logger, cfg):
    ...
    with torch.no_grad():
        ...
        frame_list = []
        pred_list = []
        for i, (v_input, feat_path) in enumerate(dataloader):
            v_input = v_input.squeeze()
            video_name = os.path.basename(feat_path[0]).split('_i3d')[0]
            ...
            pred_list.append(np.repeat(logits_np, 16))

        time_elapsed = time.time() - st
        pred_np = pred.cpu().detach().numpy()
        pred = list(pred_np)
        pred16 = np.repeat(pred, 16)

        if gt is not None:
            fpr, tpr, _ = roc_curve(list(gt), pred16)
            roc_auc = auc(fpr, tpr)
            pre, rec, th = precision_recall_curve(list(gt), pred16)
            ap_auc = auc(rec, pre)

            frame_classify = [1 if pred >= 0.49 else 0 for pred in pred16]  # 16seg
            f1 = f1_score(list(gt), frame_classify)

            logger.info('offline AUC:{:.4f} AP:{:.4f} F1-Score:{:.4f}| Complete in {:.0f}m {:.0f}s\n'.format(
                roc_auc, ap_auc, f1, time_elapsed // 60, time_elapsed % 60))
